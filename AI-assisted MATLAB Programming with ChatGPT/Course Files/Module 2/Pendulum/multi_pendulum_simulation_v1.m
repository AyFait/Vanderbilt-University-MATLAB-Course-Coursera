function multi_pendulum_simulation_v1(N, theta0, dt, Tsim)
    % Validate inputs
    if N < 1
...
function animate_pendulums(T, theta, L)
    ...
    % Set axis limits
    max_length = max(L);
    axis([-max_length-0.2 max_length+0.2 -max_length-0.2 0.2]);
    ...
    for i = 1:N
        x = L(i) * sin(theta(i, 1));
        y = -L(i) * cos(theta(i, 1));
        pendulum_lines(i) = plot([0 x], [0 y], 'LineWidth', 2);
        pendulum_masses(i) = plot(x, y, 'o', 'MarkerFaceColor', rand(1,3), 'MarkerEdgeColor', 'k');
    end
