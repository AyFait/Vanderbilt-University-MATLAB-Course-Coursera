...
if pins(index) == 10                    % strike
    ...
    first = 2;                          % Next roll counts double
    if second == 2                      % If the previous frame was a strike
        first = first + 1;              % Increment first to count the cascading bonus
    end
    second = 2;                         % Set second multiplier for the frame after next
